...
(defn pub-sub!
  "Configuration-driven (<topics>) `pub-sub` infrastructure.
   Creates a publication on channel <in> with <topic-fn> and
   ...
   the processing-fn per topic, sets-up consuming loops for all subscriptions.
   The number of consuming loops per subscription is controlled by <topic->nconsumers>
   (a map from topic-key => positive integer, defaults to 1).
